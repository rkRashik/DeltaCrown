...
const TeamCreateApp = {
    ...
    cacheDom() {
        this.dom = {
            ...
            previewGame: document.getElementById('preview-game'),
            previewRegion: document.getElementById('preview-region'),
            previewDescription: document.getElementById('preview-description'),
            previewLogoImg: document.getElementById('preview-logo-img'),
            previewBannerImg: document.getElementById('preview-banner-img'),
            previewSocials: document.getElementById('preview-socials'),
    ...
    renderGameCards() {
        if (!this.dom.gameGrid || !this.state.gameConfigs) return;

        this.dom.gameGrid.innerHTML = '';

        Object.entries(this.state.gameConfigs).forEach(([code, config]) => {
            const card = document.createElement('div');
            card.className = 'game-card';
            card.dataset.game = code;
            
            card.innerHTML = `
                <div class="game-icon" style="background: ${config.color}">
                    ${config.icon}
                </div>
                <div class="game-name">${config.name}</div>
                <div class="game-meta">${config.roster.min}-${config.roster.max} players</div>
                <div class="game-check"><i class="fa fa-check"></i></div>
            `;

            card.addEventListener('click', () => this.selectGame(code));
    ...
    selectGame(code) {
        ...
        // Select this one
        const selectedCard = document.querySelector(`[data-game="${code}"]`);
        if (selectedCard) {
            selectedCard.classList.add('selected');
        }

        this.state.selectedGame = code;
        this.state.gameConfig = this.state.gameConfigs[code];
        
        // Update hidden select
        if (this.dom.gameSelect) {
            this.dom.gameSelect.value = code;
        }
    ...
    updateGameInfo() {
        if (!this.state.gameConfig || !this.dom.gameInfoPanel) return;
    ...
    handleBannerUpload(e) {
        ...
        this.previewImage(file, this.dom.previewBanner);
        this.updatePreviewBanner(file);
    },
    ...
    updatePreviewLogo(file) {
        if (this.dom.previewLogoImg) {
            const reader = new FileReader();
            reader.onload = (e) => {
                let img = this.dom.previewLogoImg.querySelector('img');
                if (!img) {
                    img = document.createElement('img');
                    this.dom.previewLogoImg.innerHTML = '';
                    this.dom.previewLogoImg.appendChild(img);
                }
                img.src = e.target.result;
            };
            reader.readAsDataURL(file);
    ...
    updatePreviewBanner(file) {
        if (this.dom.previewBannerImg) {
            const reader = new FileReader();
            reader.onload = (e) => {
                this.dom.previewBannerImg.style.backgroundImage = `url(${e.target.result})`;
            };
            reader.readAsDataURL(file);
        }
    },
    ...
    toggleOptionalSection() {
        if (this.dom.optionalToggle && this.dom.optionalContent) {
            this.dom.optionalToggle.classList.toggle('active');
            
            if (this.dom.optionalToggle.classList.contains('active')) {
                this.dom.optionalContent.style.display = 'block';
            } else {
                this.dom.optionalContent.style.display = 'none';
            }
        }
    },
