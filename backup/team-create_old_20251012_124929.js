...
const TeamCreateApp = {
    ...
    async handleGameChange(event) {
        ...
        // ==========================================
        try {
            console.log('[TeamCreate] Checking for existing team for game:', gameCode);
            const response = await fetch(`/teams/api/check-existing-team/?game=${gameCode}`);
            const data = await response.json();
            
            if (data.has_team && !data.can_create) {
                console.log('[TeamCreate] User already has a team for this game:', data.team);
                
                // Show warning modal
                this.showExistingTeamWarning(data.team);
                
                // Reset game selection
                this.dom.gameSelect.value = '';
                this.state.selectedGame = null;
                this.state.gameConfig = null;
                this.updateRegionOptions(null);
                return;
            ...
            console.log('[TeamCreate] User can create team for this game');
        } catch (error) {
            console.error('[TeamCreate] Error checking existing team:', error);
    ...
    async handleSubmit(e) {
        console.log('[TeamCreate] Form submit triggered');
        e.preventDefault();

        // Final validation
        if (!this.validateForm()) {
            console.log('[TeamCreate] Form validation failed');
            return;
        }

        console.log('[TeamCreate] Form validation passed');

        // ==========================================
        // CHECK FOR GAME ID BEFORE SUBMITTING
        // ==========================================
        const selectedGame = this.dom.gameSelect?.value;
        console.log('[TeamCreate] Selected game:', selectedGame);
        console.log('[TeamCreate] window.gameIDPrompt exists?', !!window.gameIDPrompt);
        
        if (selectedGame && window.gameIDPrompt) {
            try {
                console.log('[TeamCreate] Checking if user has game ID for:', selectedGame);
                
                // Check if user already has game ID
                const response = await fetch(`/user/api/profile/get-game-id/?game=${selectedGame}`);
                console.log('[TeamCreate] API response status:', response.status);
                
                const data = await response.json();
                console.log('[TeamCreate] API response data:', data);
                
                if (!data.has_game_id) {
                    console.log('[TeamCreate] User missing game ID, showing prompt');
                    
                    // Show game ID prompt and wait for completion
                    const success = await window.gameIDPrompt.prompt(selectedGame);
                    
                    if (!success) {
                        console.log('[TeamCreate] User cancelled game ID prompt');
                        this.showError('Game ID is required to create a team for this game');
                        return; // Stop form submission
                    }
                    
                    console.log('[TeamCreate] Game ID saved successfully');
                } else {
                    console.log('[TeamCreate] User already has game ID');
                }
            } catch (error) {
                console.error('[TeamCreate] Error checking game ID:', error);
    ...
    // Show warning modal when user tries to select a game they already have a team for
    showExistingTeamWarning(existingTeam) {
        // Remove existing modal if any
        const existingModal = document.getElementById('existing-team-modal');
        if (existingModal) {
        ...
        modal.id = 'existing-team-modal';
        modal.className = 'game-id-modal-overlay active';
        modal.innerHTML = `
            <div class="game-id-modal">
                <button class="game-id-modal-close" aria-label="Close">&times;</button>
                
                <div class="game-id-modal-header">
                    <div class="game-id-modal-icon" style="background: linear-gradient(135deg, #ff4757 0%, #ff6b81 100%);">
                        <i class="fas fa-exclamation-triangle"></i>
                    </div>
                    <h2 class="game-id-modal-title">Already in a Team</h2>
                    <p class="game-id-modal-subtitle">You can only have one active team per game</p>
                </div>
                
                <div class="game-id-modal-body">
                    <div class="existing-team-info">
                        <div class="existing-team-card">
                            ${existingTeam.logo ? 
                                `<img src="${existingTeam.logo}" alt="${existingTeam.name}" class="existing-team-logo">` :
                                `<div class="existing-team-logo-placeholder"><i class="fas fa-shield-alt"></i></div>`
                            }
                            <div class="existing-team-details">
                                <h3>${existingTeam.name}</h3>
                                <p class="team-tag">${existingTeam.tag}</p>
                                <p class="team-role">
                                    ${existingTeam.is_captain ? 
                                        '<i class="fas fa-crown"></i> Captain' :
                                        '<i class="fas fa-user"></i> ' + existingTeam.role
                                    }
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="game-id-privacy-notice" style="border-color: rgba(255, 71, 87, 0.3); background: rgba(255, 71, 87, 0.1);">
                        <i class="fas fa-info-circle" style="color: #ff4757;"></i>
                        <div>
                            <p><strong>One Team Per Game Rule:</strong></p>
                            <p>To create a new team for this game, you must first leave your current team. You can:</p>
                            <ul style="margin: 8px 0 0 20px; line-height: 1.8;">
                                <li>Leave the team and create a new one</li>
                                <li>Choose a different game</li>
                                <li>Manage your existing team instead</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="game-id-modal-footer">
                    ...
                    <a href="/teams/${existingTeam.slug}/" class="game-id-btn" style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); text-decoration: none; color: #1a1a2e;">
                        <i class="fas fa-users"></i>
                        View My Team
                    </a>
                    <a href="/teams/${existingTeam.slug}/leave/" class="game-id-btn" style="background: linear-gradient(135deg, #ff4757 0%, #ff6b81 100%); text-decoration: none; color: white;">
        ...
        // Bind close events
        const closeBtn = modal.querySelector('.game-id-modal-close');
        const cancelBtn = modal.querySelector('.game-id-btn-cancel');
