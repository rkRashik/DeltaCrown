"""
Update all games with real 2025 ranks/tiers
"""
import os
import django
...
# Real 2025 competitive ranks per game
REAL_RANKS = {
    'Valorant': [
...
print("Updating all games with real 2025 competitive ranks...\n")

for game_name, ranks in REAL_RANKS.items():
    # Try exact match first
    game = Game.objects.filter(name=game_name).first()
    
    # If not found, try case-insensitive partial match
    if not game:
        search_name = game_name.replace('®', '').replace('™', '').replace(':', '').strip()
        game = Game.objects.filter(name__icontains=search_name.split()[0]).first()
    
    if game:
        game.available_ranks = ranks
        game.save()
        print(f"OK {game.name}: {len(ranks)} ranks")
    else:
        print(f"SKIP Game not found: {game_name}")

print("\nDone! All games updated with real ranks!")
