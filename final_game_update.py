"""
Final update script with correct game names from database
"""
import os, django
...
for game_name, data in GAME_DATA.items():
    try:
        ...
        server_icon = '🌐' if data['has_servers'] else '🌍'
        rank_icon = '🏆' if data['has_rank_system'] else '📊'
        
        print(f"✓ {game.name:30} {server_icon} Servers: {'Yes' if data['has_servers'] else 'No ':3}  {rank_icon} Ranks: {rank_count:2}")
        success_count += 1
    except Game.DoesNotExist:
        print(f"✗ {game_name:30} NOT FOUND IN DATABASE")
    except Exception as e:
        print(f"✗ {game_name:30} ERROR: {str(e)}")

print("\n" + "="*60)
print(f"COMPLETE: {success_count}/11 games updated successfully")
print("="*60)

if success_count == 11:
    print("\n📋 Summary:")
    print("  • Games with regional servers: 5 (Valorant, PUBG Mobile, MLBB, Free Fire, EA FC 26)")
    print("  • Games with global matchmaking: 6")
    print("  • Games with rank systems: 9")
    print("  • Games without ranks: 2 (EA FC 26, eFootball - use divisions/ratings)")
    print("\n✅ All games ready for Game Passport system!")
else:
    print(f"\n⚠️  Warning: Only {success_count} games were updated")
