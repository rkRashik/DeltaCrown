"""Update CSS with professional styling"""

css_content = """/* ==========================================
   PROFESSIONAL ESPORTS TOURNAMENT DETAIL
   DeltaCrown - Clean & User-Friendly Design
   ========================================== */

:root {
    --accent-red: #ff4757;
    --accent-gold: #ffd700;
    --accent-green: #00ff88;
    --accent-blue: #00d4ff;
    --dark-bg: #0a0e27;
    --darker-bg: #050816;
    --card-bg: #141b2d;
    --card-hover-bg: #1a2138;
    --border-color: rgba(255, 255, 255, 0.1);
    --text-primary: #ffffff;
    --text-secondary: #b8c5d6;
    --text-muted: #7a8699;
}

* {
    margin: 0;
    padding: 0;
    box-sizing: border-box;
}

.tournament-detail-page {
    background: var(--darker-bg);
    min-height: 100vh;
    color: var(--text-primary);
    font-family: 'Inter', sans-serif;
}

.container {
    width: 100%;
    max-width: 1400px;
    margin: 0 auto;
    padding: 0 20px;
}

/* ==========================================
   HERO SECTION - CLEAN & USER-FRIENDLY
   ========================================== */

.hero-banner {
    position: relative;
    min-height: 500px;
    display: flex;
    align-items: flex-end;
    padding-bottom: 40px;
    overflow: hidden;
}

.hero-background {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    z-index: 1;
}

.hero-image {
    width: 100%;
    height: 100%;
    object-fit: cover;
    filter: brightness(0.3);
}

.hero-gradient {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    background: linear-gradient(to bottom,
        transparent 0%,
        rgba(5, 8, 22, 0.5) 40%,
        rgba(5, 8, 22, 0.95) 100%);
}

.hero-content {
    position: relative;
    z-index: 2;
}

.hero-main {
    max-width: 900px;
}

/* Hero Status Badge */
.hero-status-badge {
    margin-bottom: 16px;
}

.status-badge-hero {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 10px 20px;
    border-radius: 30px;
    font-size: 13px;
    font-weight: 700;
    text-transform: uppercase;
    letter-spacing: 0.8px;
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3);
}

.status-live {
    background: linear-gradient(135deg, #ff4757 0%, #d63447 100%);
    color: white;
}

.status-registration {
    background: linear-gradient(135deg, #00ff88 0%, #00d46e 100%);
    color: #000;
}

.status-upcoming {
    background: linear-gradient(135deg, #00d4ff 0%, #0099cc 100%);
    color: white;
}

.status-completed {
    background: linear-gradient(135deg, #ffd700 0%, #ffaa00 100%);
    color: #000;
}

.status-published {
    background: rgba(255, 255, 255, 0.15);
    border: 1px solid rgba(255, 255, 255, 0.3);
    color: white;
}

.pulse-dot {
    width: 8px;
    height: 8px;
    border-radius: 50%;
    background: white;
    animation: pulse 2s infinite;
}

@keyframes pulse {
    0%, 100% { opacity: 1; transform: scale(1); }
    50% { opacity: 0.5; transform: scale(1.3); }
}

/* Hero Game Tag */
.hero-game-tag {
    display: inline-flex;
    align-items: center;
    gap: 10px;
    padding: 8px 16px;
    background: rgba(0, 0, 0, 0.5);
    border-radius: 8px;
    margin-bottom: 20px;
    border: 1px solid rgba(255, 255, 255, 0.1);
}

.hero-game-tag img {
    width: 24px;
    height: 24px;
    border-radius: 4px;
}

.hero-game-tag span {
    font-size: 15px;
    font-weight: 600;
    color: var(--text-primary);
}

/* Hero Title */
.hero-title {
    font-size: 48px;
    font-weight: 900;
    line-height: 1.2;
    margin-bottom: 30px;
    text-shadow: 0 2px 10px rgba(0, 0, 0, 0.5);
    font-family: 'Rajdhani', sans-serif;
}

/* Hero Info Grid - Clean Layout */
.hero-info-grid {
    display: grid;
    grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
    gap: 16px;
    max-width: 900px;
}

.hero-info-item {
    display: flex;
    align-items: center;
    gap: 14px;
    padding: 18px 20px;
    background: rgba(0, 0, 0, 0.4);
    backdrop-filter: blur(10px);
    border-radius: 12px;
    border: 1px solid rgba(255, 255, 255, 0.1);
    transition: all 0.3s ease;
}

.hero-info-item:hover {
    background: rgba(0, 0, 0, 0.6);
    border-color: var(--accent-green);
    transform: translateY(-2px);
}

.hero-info-highlight {
    background: rgba(255, 215, 0, 0.15);
    border-color: var(--accent-gold);
}

.hero-info-highlight:hover {
    background: rgba(255, 215, 0, 0.25);
}

.hero-info-icon {
    width: 44px;
    height: 44px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(0, 255, 136, 0.15);
    border-radius: 10px;
    font-size: 20px;
    color: var(--accent-green);
    flex-shrink: 0;
}

.hero-info-highlight .hero-info-icon {
    background: rgba(255, 215, 0, 0.2);
    color: var(--accent-gold);
}

.hero-info-text {
    display: flex;
    flex-direction: column;
    gap: 4px;
}

.hero-info-label {
    font-size: 11px;
    text-transform: uppercase;
    letter-spacing: 0.8px;
    color: var(--text-muted);
    font-weight: 600;
}

.hero-info-value {
    font-size: 16px;
    font-weight: 700;
    color: var(--text-primary);
    line-height: 1.2;
}

/* ==========================================
   ACTION BAR - PROFESSIONAL LAYOUT
   ========================================== */

.action-bar-premium {
    background: var(--card-bg);
    border-bottom: 1px solid var(--border-color);
    padding: 20px 0;
    position: sticky;
    top: 0;
    z-index: 100;
    box-shadow: 0 4px 20px rgba(0, 0, 0, 0.3);
}

.action-bar-wrapper {
    display: flex;
    justify-content: space-between;
    align-items: center;
    gap: 24px;
    flex-wrap: wrap;
}

.action-bar-left {
    display: flex;
    align-items: center;
    gap: 16px;
    flex-wrap: wrap;
}

.action-bar-right {
    display: flex;
    align-items: center;
    gap: 12px;
}

/* War Room Button - Modern Clean Design */
.btn-war-room-modern {
    display: inline-flex;
    align-items: center;
    gap: 12px;
    padding: 14px 28px;
    background: linear-gradient(135deg, #ff4757 0%, #d63447 100%);
    color: white;
    border-radius: 10px;
    text-decoration: none;
    font-weight: 800;
    font-size: 15px;
    letter-spacing: 1px;
    text-transform: uppercase;
    box-shadow: 0 4px 15px rgba(255, 71, 87, 0.4);
    transition: all 0.3s ease;
}

.btn-war-room-modern:hover {
    transform: translateY(-2px);
    box-shadow: 0 6px 20px rgba(255, 71, 87, 0.6);
}

.btn-icon-modern {
    font-size: 18px;
}

.btn-text-modern {
    font-size: 15px;
    font-weight: 800;
}

.btn-badge-modern {
    font-size: 20px;
    filter: drop-shadow(0 0 8px rgba(255, 255, 255, 0.3));
}

/* Register Button */
.btn-register-modern {
    display: inline-flex;
    align-items: center;
    gap: 12px;
    padding: 14px 28px;
    background: linear-gradient(135deg, #00ff88 0%, #00d46e 100%);
    color: #000;
    border-radius: 10px;
    text-decoration: none;
    font-weight: 800;
    font-size: 15px;
    letter-spacing: 1px;
    text-transform: uppercase;
    box-shadow: 0 4px 15px rgba(0, 255, 136, 0.4);
    transition: all 0.3s ease;
}

.btn-register-modern:hover {
    transform: translateY(-2px);
    box-shadow: 0 6px 20px rgba(0, 255, 136, 0.6);
}

/* Disabled Button */
.btn-disabled-modern {
    display: inline-flex;
    align-items: center;
    gap: 12px;
    padding: 14px 28px;
    background: rgba(255, 255, 255, 0.05);
    color: var(--text-muted);
    border-radius: 10px;
    font-weight: 700;
    font-size: 15px;
    letter-spacing: 1px;
    text-transform: uppercase;
    border: 1px solid var(--border-color);
    cursor: not-allowed;
    opacity: 0.6;
}

/* Status Badges */
.registration-badge {
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 8px 16px;
    background: rgba(0, 255, 136, 0.15);
    color: var(--accent-green);
    border-radius: 20px;
    font-size: 13px;
    font-weight: 600;
    border: 1px solid rgba(0, 255, 136, 0.3);
}

.slots-badge {
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 8px 16px;
    background: rgba(255, 71, 87, 0.15);
    color: var(--accent-red);
    border-radius: 20px;
    font-size: 13px;
    font-weight: 600;
    border: 1px solid rgba(255, 71, 87, 0.3);
    animation: urgentPulse 2s infinite;
}

@keyframes urgentPulse {
    0%, 100% { opacity: 1; }
    50% { opacity: 0.7; }
}

/* Action Info Card */
.action-info-card {
    display: flex;
    align-items: center;
    gap: 12px;
    padding: 12px 18px;
    background: rgba(255, 255, 255, 0.05);
    border-radius: 10px;
    border: 1px solid var(--border-color);
}

.action-countdown-card {
    background: rgba(255, 215, 0, 0.1);
    border-color: rgba(255, 215, 0, 0.3);
}

.action-card-icon {
    width: 36px;
    height: 36px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(255, 215, 0, 0.2);
    border-radius: 8px;
    font-size: 18px;
    color: var(--accent-gold);
}

.action-card-content {
    display: flex;
    flex-direction: column;
    gap: 2px;
}

.action-card-label {
    font-size: 10px;
    text-transform: uppercase;
    letter-spacing: 0.5px;
    color: var(--text-muted);
    font-weight: 600;
}

.action-card-value {
    font-family: 'Rajdhani', monospace;
    font-size: 16px;
    font-weight: 700;
    color: var(--accent-gold);
}

/* Share Button */
.action-share-btn {
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 12px 20px;
    background: rgba(255, 255, 255, 0.05);
    color: var(--text-primary);
    border: 1px solid var(--border-color);
    border-radius: 10px;
    cursor: pointer;
    font-weight: 600;
    font-size: 14px;
    transition: all 0.3s ease;
}

.action-share-btn:hover {
    background: rgba(255, 255, 255, 0.1);
    border-color: var(--accent-blue);
    transform: translateY(-2px);
}

/* ==========================================
   MAIN CONTENT - PROFESSIONAL SPACING
   ========================================== */

.main-content {
    padding: 40px 0;
}

.content-layout {
    display: grid;
    grid-template-columns: 1fr;
    gap: 32px;
}

.content-primary {
    order: 2;
}

.content-sidebar {
    order: 1;
}

/* Content Cards */
.content-card {
    background: var(--card-bg);
    border: 1px solid var(--border-color);
    border-radius: 16px;
    margin-bottom: 32px;
    overflow: hidden;
}

.card-header {
    padding: 24px 28px;
    border-bottom: 1px solid var(--border-color);
    background: rgba(255, 255, 255, 0.02);
}

.card-title {
    font-size: 22px;
    font-weight: 800;
    font-family: 'Rajdhani', sans-serif;
    color: var(--text-primary);
    display: flex;
    align-items: center;
    gap: 12px;
    text-transform: uppercase;
}

.card-title i {
    color: var(--accent-green);
    font-size: 24px;
}

.card-body {
    padding: 28px;
}

/* Countdown Section */
.countdown-section-premium {
    background: linear-gradient(135deg, rgba(255, 215, 0, 0.1) 0%, rgba(255, 71, 87, 0.1) 100%);
    border: 2px solid rgba(255, 215, 0, 0.3);
    border-radius: 16px;
    padding: 40px 32px;
    margin-bottom: 32px;
}

.countdown-header-premium {
    text-align: center;
    margin-bottom: 28px;
}

.countdown-badge {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 8px 16px;
    background: rgba(0, 0, 0, 0.4);
    border-radius: 20px;
    font-size: 12px;
    font-weight: 700;
    color: var(--accent-gold);
    margin-bottom: 12px;
}

.countdown-title-premium {
    font-size: 28px;
    font-weight: 900;
    font-family: 'Rajdhani', sans-serif;
    text-transform: uppercase;
}

.countdown-timer-premium {
    display: grid;
    grid-template-columns: repeat(7, 1fr);
    gap: 12px;
    align-items: center;
}

.countdown-block {
    text-align: center;
    background: rgba(0, 0, 0, 0.4);
    border-radius: 12px;
    padding: 20px 12px;
    border: 1px solid rgba(255, 215, 0, 0.3);
}

.countdown-number {
    font-family: 'Rajdhani', monospace;
    font-size: 36px;
    font-weight: 900;
    color: var(--accent-gold);
    line-height: 1;
}

.countdown-text {
    font-size: 11px;
    text-transform: uppercase;
    color: var(--text-muted);
    margin-top: 6px;
    font-weight: 600;
}

.countdown-divider {
    font-size: 28px;
    font-weight: 900;
    color: var(--accent-gold);
    text-align: center;
}

/* Stats Showcase */
.stats-showcase {
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    gap: 16px;
    margin-bottom: 32px;
}

.stat-box {
    background: var(--card-bg);
    border: 1px solid var(--border-color);
    border-radius: 12px;
    padding: 24px;
    display: flex;
    align-items: center;
    gap: 16px;
    transition: all 0.3s ease;
}

.stat-box:hover {
    border-color: var(--accent-green);
    transform: translateY(-2px);
    box-shadow: 0 4px 20px rgba(0, 255, 136, 0.2);
}

.stat-icon {
    width: 52px;
    height: 52px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(0, 255, 136, 0.1);
    border-radius: 12px;
    font-size: 26px;
    color: var(--accent-green);
}

.stat-box-highlight .stat-icon {
    background: rgba(255, 215, 0, 0.1);
    color: var(--accent-gold);
}

.stat-value {
    font-size: 28px;
    font-weight: 900;
    font-family: 'Rajdhani', sans-serif;
    color: var(--text-primary);
}

.stat-label {
    font-size: 12px;
    text-transform: uppercase;
    color: var(--text-muted);
    font-weight: 600;
}

/* Info Grid */
.info-grid {
    display: grid;
    grid-template-columns: 1fr;
    gap: 20px;
    margin-top: 24px;
}

.info-box {
    display: flex;
    align-items: center;
    gap: 16px;
    padding: 20px;
    background: rgba(255, 255, 255, 0.03);
    border-radius: 12px;
    border: 1px solid var(--border-color);
}

.info-icon {
    width: 44px;
    height: 44px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(0, 255, 136, 0.1);
    border-radius: 10px;
    font-size: 20px;
    color: var(--accent-green);
}

.info-label {
    font-size: 12px;
    text-transform: uppercase;
    color: var(--text-muted);
    font-weight: 600;
}

.info-value {
    font-size: 16px;
    font-weight: 700;
    color: var(--text-primary);
    margin-top: 4px;
}

/* Sidebar Widgets */
.sidebar-widget {
    background: var(--card-bg);
    border: 1px solid var(--border-color);
    border-radius: 16px;
    margin-bottom: 24px;
    overflow: hidden;
}

.widget-header {
    padding: 20px 24px;
    background: rgba(255, 255, 255, 0.03);
    border-bottom: 1px solid var(--border-color);
    display: flex;
    align-items: center;
    gap: 12px;
}

.widget-icon {
    width: 36px;
    height: 36px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(0, 255, 136, 0.1);
    border-radius: 10px;
    font-size: 18px;
    color: var(--accent-green);
}

.widget-title {
    font-size: 16px;
    font-weight: 800;
    font-family: 'Rajdhani', sans-serif;
    text-transform: uppercase;
}

.widget-body {
    padding: 24px;
}

/* Info Items Modern */
.info-item-modern {
    display: flex;
    align-items: flex-start;
    gap: 14px;
    padding: 16px 0;
    border-bottom: 1px solid rgba(255, 255, 255, 0.05);
}

.info-item-modern:last-child {
    border-bottom: none;
    padding-bottom: 0;
}

.info-item-modern:first-child {
    padding-top: 0;
}

.info-icon-modern {
    width: 36px;
    height: 36px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(0, 255, 136, 0.1);
    border-radius: 8px;
    font-size: 16px;
    color: var(--accent-green);
    flex-shrink: 0;
}

.info-label-modern {
    font-size: 11px;
    text-transform: uppercase;
    color: var(--text-muted);
    font-weight: 600;
    margin-bottom: 4px;
}

.info-value-modern {
    font-size: 14px;
    font-weight: 700;
    color: var(--text-primary);
}

.info-highlight {
    background: rgba(0, 255, 136, 0.05);
    border-radius: 10px;
    padding: 16px !important;
    border: 1px solid rgba(0, 255, 136, 0.2);
    margin: 8px 0;
}

/* Stat Items Modern */
.stat-item-modern {
    display: flex;
    align-items: center;
    gap: 14px;
    padding: 16px 0;
    border-bottom: 1px solid rgba(255, 255, 255, 0.05);
}

.stat-item-modern:last-child {
    border-bottom: none;
    padding-bottom: 0;
}

.stat-item-modern:first-child {
    padding-top: 0;
}

.stat-icon-wrapper {
    width: 40px;
    height: 40px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(0, 255, 136, 0.1);
    border-radius: 10px;
    font-size: 18px;
    color: var(--accent-green);
}

.stat-alert .stat-icon-wrapper {
    background: rgba(255, 71, 87, 0.1);
    color: var(--accent-red);
}

.stat-prize .stat-icon-wrapper {
    background: rgba(255, 215, 0, 0.1);
    color: var(--accent-gold);
}

.stat-label-modern {
    font-size: 11px;
    text-transform: uppercase;
    color: var(--text-muted);
    font-weight: 600;
}

.stat-value-modern {
    font-size: 16px;
    font-weight: 900;
    font-family: 'Rajdhani', sans-serif;
    color: var(--text-primary);
}

.stat-progress {
    width: 100%;
    height: 4px;
    background: rgba(255, 255, 255, 0.1);
    border-radius: 2px;
    overflow: hidden;
    margin-top: 8px;
}

.stat-progress-bar {
    height: 100%;
    background: linear-gradient(90deg, var(--accent-green) 0%, var(--accent-blue) 100%);
    transition: width 0.5s ease;
}

/* Timeline */
.timeline-modern {
    padding: 20px 0;
}

.timeline-event {
    position: relative;
    padding-left: 40px;
    margin-bottom: 24px;
}

.timeline-event::before {
    content: '';
    position: absolute;
    left: 12px;
    top: 40px;
    width: 2px;
    height: calc(100% + 24px);
    background: var(--border-color);
}

.timeline-event:last-child::before {
    display: none;
}

.timeline-marker {
    position: absolute;
    left: 0;
    top: 8px;
    width: 24px;
    height: 24px;
    border-radius: 50%;
    background: var(--card-bg);
    border: 3px solid var(--border-color);
}

.timeline-active .timeline-marker {
    background: var(--accent-green);
    border-color: var(--accent-green);
    box-shadow: 0 0 20px rgba(0, 255, 136, 0.5);
}

.timeline-card {
    background: rgba(255, 255, 255, 0.03);
    border: 1px solid var(--border-color);
    border-radius: 12px;
    padding: 16px;
}

.timeline-active .timeline-card {
    border-color: var(--accent-green);
    background: rgba(0, 255, 136, 0.05);
}

.timeline-date {
    font-size: 12px;
    color: var(--text-muted);
    margin-bottom: 6px;
    display: flex;
    align-items: center;
    gap: 6px;
}

.timeline-title {
    font-size: 16px;
    font-weight: 700;
    color: var(--text-primary);
}

/* Prizes */
.prizes-grid {
    display: grid;
    grid-template-columns: 1fr;
    gap: 16px;
}

.prize-item {
    display: flex;
    align-items: center;
    gap: 16px;
    padding: 24px;
    background: rgba(255, 215, 0, 0.05);
    border: 1px solid rgba(255, 215, 0, 0.2);
    border-radius: 12px;
    transition: all 0.3s ease;
}

.prize-item:hover {
    border-color: var(--accent-gold);
    transform: translateY(-2px);
}

.prize-medal {
    font-size: 48px;
}

.prize-rank {
    font-size: 16px;
    font-weight: 700;
    text-transform: uppercase;
}

.prize-amount {
    font-size: 24px;
    font-weight: 900;
    font-family: 'Rajdhani', sans-serif;
    color: var(--accent-gold);
    margin-left: auto;
}

/* Rules */
.rules-container {
    display: grid;
    gap: 24px;
}

.rule-section {
    padding: 24px;
    background: rgba(255, 255, 255, 0.03);
    border-radius: 12px;
    border: 1px solid var(--border-color);
}

.rule-heading {
    font-size: 18px;
    font-weight: 800;
    font-family: 'Rajdhani', sans-serif;
    margin-bottom: 12px;
    display: flex;
    align-items: center;
    gap: 10px;
    text-transform: uppercase;
}

.rule-heading i {
    color: var(--accent-green);
}

.rule-content {
    font-size: 14px;
    line-height: 1.7;
    color: var(--text-secondary);
}

.tournament-description {
    font-size: 15px;
    line-height: 1.7;
    color: var(--text-secondary);
    margin-bottom: 24px;
}

/* Participants */
.participants-note {
    display: flex;
    align-items: flex-start;
    gap: 12px;
    padding: 18px;
    background: rgba(0, 212, 255, 0.1);
    border: 1px solid rgba(0, 212, 255, 0.3);
    border-radius: 12px;
    font-size: 14px;
    color: var(--text-secondary);
}

.participants-note i {
    color: var(--accent-blue);
    font-size: 18px;
    margin-top: 2px;
}

.inline-link {
    color: var(--accent-blue);
    text-decoration: underline;
    font-weight: 600;
}

.participant-count {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    min-width: 32px;
    height: 32px;
    padding: 0 12px;
    background: rgba(0, 255, 136, 0.2);
    color: var(--accent-green);
    border-radius: 16px;
    font-weight: 900;
    font-size: 14px;
}

.btn-download-rules {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 10px 18px;
    background: rgba(0, 255, 136, 0.1);
    color: var(--accent-green);
    border-radius: 10px;
    text-decoration: none;
    font-weight: 600;
    font-size: 14px;
    transition: all 0.3s ease;
}

.btn-download-rules:hover {
    background: rgba(0, 255, 136, 0.2);
    transform: translateY(-2px);
}

/* Organizer Card */
.organizer-card-modern {
    display: flex;
    align-items: center;
    gap: 16px;
}

.organizer-avatar-modern {
    width: 56px;
    height: 56px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(0, 255, 136, 0.1);
    border-radius: 12px;
    font-size: 24px;
    color: var(--accent-green);
}

.organizer-name-modern {
    font-size: 16px;
    font-weight: 700;
    margin-bottom: 6px;
}

.organizer-contact-modern {
    display: inline-flex;
    align-items: center;
    gap: 6px;
    font-size: 13px;
    color: var(--accent-blue);
    text-decoration: none;
    font-weight: 600;
}

.organizer-contact-modern:hover {
    color: var(--accent-green);
}

/* Share Modal */
.share-modal {
    display: none;
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    z-index: 1000;
    align-items: center;
    justify-content: center;
}

.share-modal.active {
    display: flex;
}

.modal-backdrop {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    background: rgba(0, 0, 0, 0.8);
    backdrop-filter: blur(10px);
}

.modal-container {
    position: relative;
    background: var(--card-bg);
    border: 1px solid var(--border-color);
    border-radius: 16px;
    width: 90%;
    max-width: 500px;
    box-shadow: 0 20px 60px rgba(0, 0, 0, 0.5);
    z-index: 1001;
}

.modal-header {
    padding: 24px;
    border-bottom: 1px solid var(--border-color);
    display: flex;
    justify-content: space-between;
    align-items: center;
}

.modal-title {
    font-size: 20px;
    font-weight: 800;
    font-family: 'Rajdhani', sans-serif;
}

.modal-close {
    width: 32px;
    height: 32px;
    display: flex;
    align-items: center;
    justify-content: center;
    background: rgba(255, 255, 255, 0.05);
    border: 1px solid var(--border-color);
    border-radius: 8px;
    cursor: pointer;
    transition: all 0.2s ease;
}

.modal-close:hover {
    background: rgba(255, 71, 87, 0.2);
    border-color: var(--accent-red);
    color: var(--accent-red);
}

.modal-body {
    padding: 24px;
}

.share-options {
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    gap: 12px;
}

.share-option {
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 8px;
    padding: 20px;
    background: rgba(255, 255, 255, 0.05);
    border: 1px solid var(--border-color);
    border-radius: 12px;
    cursor: pointer;
    transition: all 0.3s ease;
    font-weight: 600;
    font-size: 14px;
}

.share-option i {
    font-size: 28px;
}

.share-facebook {
    background: rgba(59, 89, 152, 0.1);
    border-color: #3b5998;
    color: #3b5998;
}

.share-twitter {
    background: rgba(29, 161, 242, 0.1);
    border-color: #1da1f2;
    color: #1da1f2;
}

.share-whatsapp {
    background: rgba(37, 211, 102, 0.1);
    border-color: #25d366;
    color: #25d366;
}

.share-copy {
    background: rgba(0, 255, 136, 0.1);
    border-color: var(--accent-green);
    color: var(--accent-green);
}

.share-option:hover {
    transform: translateY(-2px);
}

/* ==========================================
   RESPONSIVE DESIGN
   ========================================== */

@media (min-width: 768px) {
    .container {
        padding: 0 32px;
    }

    .hero-banner {
        min-height: 550px;
    }

    .hero-title {
        font-size: 56px;
    }

    .hero-info-grid {
        grid-template-columns: repeat(4, 1fr);
    }

    .action-bar-wrapper {
        flex-wrap: nowrap;
    }

    .stats-showcase {
        grid-template-columns: repeat(4, 1fr);
        gap: 20px;
    }

    .content-layout {
        grid-template-columns: 1fr 380px;
        gap: 40px;
    }

    .content-primary {
        order: 1;
    }

    .content-sidebar {
        order: 2;
    }

    .info-grid {
        grid-template-columns: repeat(2, 1fr);
    }

    .prizes-grid {
        grid-template-columns: repeat(2, 1fr);
    }

    .share-options {
        grid-template-columns: repeat(4, 1fr);
    }

    .countdown-timer-premium {
        gap: 16px;
    }

    .countdown-block {
        padding: 24px 16px;
    }

    .countdown-number {
        font-size: 44px;
    }
}

@media (min-width: 1024px) {
    .main-content {
        padding: 50px 0;
    }

    .hero-banner {
        min-height: 600px;
    }

    .hero-title {
        font-size: 64px;
    }
}

@media print {
    .action-bar-premium,
    .share-modal,
    .action-share-btn {
        display: none !important;
    }
}
"""

# Write CSS
output_path = r'G:\My Projects\WORK\DeltaCrown\static\tournaments\css\tournament-detail-modern.css'
with open(output_path, 'w', encoding='utf-8') as f:
    f.write(css_content)

print(f"✅ Written {len(css_content)} characters to CSS file")
print("✅ Professional styling applied!")
