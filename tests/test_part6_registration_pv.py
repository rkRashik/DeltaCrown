...
def test_pv_populated_on_solo_registration(django_user_model):
    ...
    t = Tournament.objects.create(name="E-Cup P6", game="efootball")
...
def test_admin_duplicate_transaction_id_block(admin_user, django_user_model):
    ...
    t = Tournament.objects.create(name="E-Cup P6 Dups", game="efootball")

    reg1 = register_efootball_player(SoloRegistrationInput(tournament_id=t.id, user_id=p1.id, payment_method="bkash", payment_reference="DUP-TX", payer_account_number="017...", amount_bdt=100))
    reg2 = register_efootball_player(SoloRegistrationInput(tournament_id=t.id, user_id=p2.id, payment_method="bkash", payment_reference="DUP-TX", payer_account_number="018...", amount_bdt=100))
