...
@pytest.fixture
def redis_rate_limit_config(test_namespace):
    """
    ...
    config = {
        ...
        # Use test namespace for all Redis keys
        'WS_RATE_KEY_PREFIX': test_namespace,
    }
    
    with patch.dict('django.conf.settings.__dict__', config, clear=False):
        # Also patch the settings module attributes directly
        import django.conf
...
def fast_cooldown():
    ...
    """
    config = {
        'WS_RATE_MSG_WINDOW_MS': 200,  # 200ms message rate window
        'WS_RATE_CONN_TTL_SEC': 1,     # 1s connection TTL (for cleanup tests)
    }
