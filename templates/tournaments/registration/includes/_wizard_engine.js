...
(function() {
    ...
    function initCspSafeBindings() {
        if (document.body && document.body.dataset.registrationCspBound === 'true') {
            return;
        }
        if (document.body) {
            document.body.dataset.registrationCspBound = 'true';
        }
        ...
        document.addEventListener('input', handleCspInput);
        document.addEventListener('change', handleCspChange);

        document.querySelectorAll('[data-drop-input]').forEach(function(zone) {
            ...
            zone.addEventListener('drop', function(event) {
                handleFileDrop(event, zone.dataset.dropInput);
                checkStep('extras');
            });
        });
    }
    ...
    function handleDelegatedClick(event) {
        if (event.target && event.target.id === 'member-modal-overlay') {
            closeMemberModal(event);
            return;
        }

        const goToStepIndexBtn = event.target.closest('[data-go-to-step-index]');
        if (goToStepIndexBtn) {
            const idx = parseInt(goToStepIndexBtn.dataset.goToStepIndex || '', 10);
            if (!Number.isNaN(idx)) goToStep(idx);
            return;
        }

        const goToStepKeyBtn = event.target.closest('[data-go-to-step-key]');
        if (goToStepKeyBtn) {
            const key = goToStepKeyBtn.dataset.goToStepKey || '';
            if (key) goToStepByKey(key);
            return;
        }

        const accordionBtn = event.target.closest('[data-accordion-target]');
        if (accordionBtn) {
            const accordionId = accordionBtn.dataset.accordionTarget || '';
            if (accordionId) toggleAccordion(accordionId);
            return;
        }

        const mobileTabBtn = event.target.closest('.mobile-tab[data-mobile-method]');
        if (mobileTabBtn) {
            const method = mobileTabBtn.dataset.mobileMethod || '';
            if (method) selectMobileMethod(method);
            return;
        }
        ...
        switch (actionEl.dataset.action) {
            ...
            case 'add-guest-member':
                addGuestMember();
                break;
            case 'remove-guest-member':
                removeGuestMember(actionEl);
                break;
            case 'copy-payment-number':
                copyNumber(actionEl);
                break;
            case 'switch-modal-tab':
                if (actionEl.dataset.modalTabTarget) {
                    switchModalTab(actionEl.dataset.modalTabTarget);
                }
                break;
            default:
                break;
    ...
    function handleCspInput(event) {
        const el = event.target;
        if (!el) return;

        const isRegistrationField = el.closest('#registration-form') || el.closest('#member-modal-overlay');
        if (!isRegistrationField) return;
        ...
        if (el.matches('input[name="phone"], input[name="captain_whatsapp"], input[name="whatsapp"], input[name="discord"], input[name^="comm_"]')) {
            applyInputValidation(el);
        }

        if (el.matches('[name="guest_team_name"], [name="guest_team_tag"], [name^="member_game_id_"], [name^="member_display_name_"]')) {
            checkStep('guest_team');
        }

        if (el.closest('[data-wizard-step="profile"]')) {
            checkStep('profile');
        }
        if (el.closest('[data-wizard-step="coordinator"]')) {
            checkStep('coordinator');
        }
        if (el.closest('[data-wizard-step="extras"]')) {
            checkStep('extras');
        }
        if (el.id === 'payment_mobile') {
            checkStep('payment');
        }
    }

    function handleCspChange(event) {
        const el = event.target;
        if (!el) return;
        ...
        if (el.matches('input[name="coordinator_is_self"]')) {
            toggleCoordinatorMode(el.value === 'true');
            checkStep('coordinator');
            return;
        }

        if (el.matches('input[name="coordinator_member_id"]')) {
            selectCoordinatorMember(el);
            return;
        }

        if (el.matches('input[name="coordinator_role"], input[name="preferred_contact"]')) {
            checkStep('coordinator');
        }

        if (el.matches('input[name="team_id"]')) {
            checkStep('select_team');
        }

        if (el.matches('input[name="payment_method"]')) {
            selectPaymentMethod(el.value);
            return;
        }

        if (el.matches('select[name^="member_role_"]')) {
            updateRosterCounts();
            checkStep('guest_team');
        }

        if (el.matches('input[name="payment_proof"]')) {
            handlePaymentProof(el);
            checkStep('payment');
        }

        if (el.matches('input[data-custom-file-question]')) {
            const labelId = el.dataset.fileLabelId;
            const labelEl = labelId ? document.getElementById(labelId) : null;
            if (labelEl) {
                labelEl.textContent = el.files && el.files[0] ? el.files[0].name : 'Click or drag to upload';
            }
            checkStep('extras');
        }

        if (el.matches('input[name="team_logo"]')) {
            previewUpload(el, 'logo');
            checkStep('extras');
        }

        if (el.matches('input[name="team_banner"]')) {
            previewUpload(el, 'banner');
            checkStep('extras');
        }

        if (el.id === 'terms-checkbox') {
            validateTerms();
            updateTermsCardStyle();
            return;
        }
        ...
        if (el.closest('[data-wizard-step="payment"]')) {
            checkStep('payment');
        }
    }
    ...
    function clearHiddenRequiredAttributes() {
        document.querySelectorAll('[data-wizard-step]').forEach(function(stepEl) {
            ...
            stepEl.querySelectorAll('input, select, textarea').forEach(function(inp) {
                ...
                if (!isHidden && inp.dataset.wasRequired === 'true') {
    ...
    function clearStepValidationUI(stepEl) {
        ...
        stepEl.querySelectorAll('.field-inline-error').forEach(function(err) {
            err.textContent = '';
    ...
    function getOrCreateFieldErrorNode(input) {
        ...
        node.className = 'field-inline-error hidden mt-1.5 text-[11px] text-red-300';
        anchor.parentElement.insertBefore(node, anchor.nextSibling);
        return node;
    }

    function showInlineFieldError(input, message) {
    ...
    function getFieldValidationMessage(el) {
        ...
        if (el.id === 'trxid' && value && !/^[A-Za-z0-9]{10}$/.test(value)) {
            return 'Enter a valid 10-character transaction ID.';
        }
        ...
        if (el.type === 'tel' && value && !/^[\d+\-() ]{7,20}$/.test(value)) {
            return 'Enter a valid phone number.';
        }

        if (el.validity && value && !el.validity.valid) {
    ...
    function updateFooterButtons() {
        ...
        const currentStepValid = !!stepValidity[currentStep];

        if (btnNext) {
            btnNext.style.display = isLastStep ? 'none' : 'flex';
            btnNext.disabled = !currentStepValid;
            btnNext.classList.toggle('opacity-50', !currentStepValid);
            btnNext.classList.toggle('cursor-not-allowed', !currentStepValid);
            btnNext.classList.toggle('pointer-events-none', !currentStepValid);
        }

        if (btnSubmit) {
            btnSubmit.style.display = isLastStep ? 'flex' : 'none';
            btnSubmit.disabled = !currentStepValid;
            btnSubmit.classList.toggle('opacity-50', !currentStepValid);
            btnSubmit.classList.toggle('cursor-not-allowed', !currentStepValid);
            btnSubmit.classList.toggle('pointer-events-none', !currentStepValid);
        }
    }
        ...
        row.innerHTML = `
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    <select name="member_role_${idx}" class="input-premium text-xs w-28 py-2 px-3">
                ...
                <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input type="text" name="member_game_id_${idx}" class="input-premium text-sm" placeholder="${runtimeConfig.gameIdLabel}" required>
                    <input type="text" name="member_display_name_${idx}" class="input-premium text-sm" placeholder="Display Name" required>
                </div>
                <button type="button" data-action="remove-guest-member" class="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 hover:text-red-400 hover:border-red-400/30 transition-all shrink-0" title="Remove">
    ...
    function openMemberModal(cardEl) {
        ...
        body.innerHTML = `
            ...
            <div class="flex gap-1 mt-3 mb-3 p-1 rounded-xl bg-black/40 border border-white/5 flex-wrap">
                <button type="button" data-modal-tab-btn="edit" data-action="switch-modal-tab" data-modal-tab-target="edit" class="px-3 py-2 rounded-lg text-xs font-bold transition-all" style="background: rgba(var(--accent-rgb), 0.15); color: var(--accent); border: 1px solid rgba(var(--accent-rgb), 0.2);">
                    <i data-lucide="settings-2" class="w-3 h-3 inline -mt-0.5 mr-1"></i>Assignment
                </button>
                <button type="button" data-modal-tab-btn="passport" data-action="switch-modal-tab" data-modal-tab-target="passport" class="px-3 py-2 rounded-lg text-xs font-bold text-gray-500 hover:text-white transition-all" style="background: transparent; border: 1px solid transparent;">
                    <i data-lucide="id-card" class="w-3 h-3 inline -mt-0.5 mr-1"></i>Game Passport
                </button>
                <button type="button" data-modal-tab-btn="player-info" data-action="switch-modal-tab" data-modal-tab-target="player-info" class="px-3 py-2 rounded-lg text-xs font-bold text-gray-500 hover:text-white transition-all" style="background: transparent; border: 1px solid transparent;">
                    <i data-lucide="user" class="w-3 h-3 inline -mt-0.5 mr-1"></i>Player Info
                </button>
                <button type="button" data-modal-tab-btn="stats" data-action="switch-modal-tab" data-modal-tab-target="stats" class="px-3 py-2 rounded-lg text-xs font-bold text-gray-500 hover:text-white transition-all" style="background: transparent; border: 1px solid transparent;">
    ...
    function handleSubmit(event) {
        ...
        // Submit as full navigation so the new document gets a fresh CSP nonce.
    ...
    function handleLiveChange(e) {
        ...
        if (!valid && stepEl === document.querySelector(`[data-wizard-step="${stepsConfig[currentStep].key}"]`)) {
            const invalid = getFirstVisibleInvalidInput(stepEl);
            if (invalid) {
                invalid.dataset.touched = 'true';
                applyFieldFeedback(invalid, true);
            }
        }
    }

    function applyFieldFeedback(el, forceShowError) {
        if (!el) return;
        ...
        const isTouched = forceShowError || el.dataset.touched === 'true';
        const message = getFieldValidationMessage(el);

        if (isTouched && message) {
            showInlineFieldError(el, message);
            el.style.borderColor = 'rgba(248, 113, 113, 0.5)';
            el.style.boxShadow = '0 0 0 1px rgba(248, 113, 113, 0.14)';
            return;
        }
    ...
    function initPreferredContactValidation() {
        ...
        Object.entries(contactInputMap).forEach(function(entry) {
            var key = entry[0];
            var selector = entry[1];
            document.querySelectorAll(selector).forEach(function(el) {
                el.dataset.contactField = 'true';
                el.dataset.contactKey = key;
                if (el.hasAttribute('required')) {
                    el.dataset.baseRequired = 'true';
                }
            });
        });

        var requiredContactChannels = Array.isArray(runtimeConfig.requiredContactChannels)
            ? runtimeConfig.requiredContactChannels
            : [];
        requiredContactChannels.forEach(function(rawKey) {
            var key = String(rawKey || '').trim().toLowerCase();
            var selector = contactInputMap[key];
            if (!selector) return;
            document.querySelectorAll(selector).forEach(function(el) {
                el.dataset.baseRequired = 'true';
        ...
        function applyContactRequired(selectedValue) {
            // Apply required state from base TOC config plus selected preferred channel.
            Object.entries(contactInputMap).forEach(function(entry) {
                var key = entry[0];
                var selector = entry[1];
                document.querySelectorAll(selector).forEach(function(el) {
                    var isBaseRequired = el.dataset.baseRequired === 'true';
                    var isPreferredRequired = key === selectedValue;
                    if (isBaseRequired || isPreferredRequired) {
                        el.setAttribute('required', '');
                    } else {
