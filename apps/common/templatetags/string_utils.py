...
Each helper aims to be defensive so optional data from views or API payloads
never causes template rendering to explode.
"""
...
def coalesce(value, fallback: str = "") -> str:
    """Return the first truthy value and otherwise the fallback."""
...
def slugify_filter(value, allow_fallback: bool = True) -> str:
    """Safe wrapper around :func:django.utils.text.slugify."""
    if not value:
