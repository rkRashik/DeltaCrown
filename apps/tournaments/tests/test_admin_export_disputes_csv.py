...
from apps.tournaments.admin import export_disputes_csv
from apps.tournaments.models import Tournament

pytestmark = pytest.mark.django_db


def test_export_disputes_csv_header_only():
    """
    Call the admin action with an empty queryset to assert headers and CSV shape,
    without relying on the exact Dispute schema or required fields.
    We reuse Tournament.objects.none() to supply a valid QuerySet.
    """
    qs = Tournament.objects.none()
    resp = export_disputes_csv(modeladmin=None, request=None, queryset=qs)
