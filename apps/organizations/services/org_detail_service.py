...
def _safe_game_icon(team):
    ...
    Uses the Game model's ImageField so the URL is generated by the
    configured storage backend (FileSystemStorage locally,
    MediaCloudinaryStorage in production) rather than a hardcoded
    /media/ prefix that breaks on Cloudinary.
    """
    game_id = getattr(team, 'game_id', None)
    if not game_id:
        return ''
    try:
        from apps.games.models.game import Game
        game = Game.objects.filter(id=game_id).only('icon').first()
