...
class Organization(models.Model):
    ...
    def save(self, *args, **kwargs):
        """
        Auto-generate slug and public_id if not provided.
        
        Note: UUID is auto-generated by field default on creation.
        """
        # Auto-generate slug from name
        if not self.slug:
            self.slug = slugify(self.name)
        
        # Auto-generate public_id with collision retry
        if not self.public_id:
            from apps.organizations.utils.ids import generate_public_id
            
            # Try up to 5 times to generate unique public_id
            for _ in range(5):
                candidate_id = generate_public_id(prefix="ORG", length=10)
                if not Organization.objects.filter(public_id=candidate_id).exists():
                    self.public_id = candidate_id
                    break
            else:
                # If all 5 attempts collide (extremely unlikely), raise error
                raise ValueError("Failed to generate unique public_id after 5 attempts")
        
        super().save(*args, **kwargs)
    
    def get_absolute_url(self):
        """
        Return canonical URL for organization detail page.
        
        This is the public-facing page with org info, squads, activity, etc.
    ...
    def get_hub_url(self):
        """
        Return URL for organization hub (internal management page).
        
        Only accessible to org managers/CEO. Contains feeds, team management, etc.
        
        Example: /orgs/syntax/hub/
        """
        return reverse('organizations:org_hub', kwargs={'org_slug': self.slug})
