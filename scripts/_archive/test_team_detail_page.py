...
if not teams:
    print("âŒ No teams found in database. Please create a team first.")
    exit(1)

team = teams[0]
print(f"ðŸŽ® Testing with team: {team.name} ({team.slug})")

# Get test users
users = User.objects.filter(is_active=True)[:4]
if not users:
    print("âŒ No active users found. Please create users first.")
    exit(1)

print(f"\nðŸ“‹ Testing team detail page: /teams/{team.slug}/\n")
...
if response.status_code == 200:
    if b'Reverse for' not in response.content:
        ...
        print(f"   âŒ Status: {response.status_code} - NoReverseMatch error found!")
