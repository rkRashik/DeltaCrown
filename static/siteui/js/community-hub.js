...
;(function () {
  ...
  function renderPost(p) {
    ...
    if (CFG.currentUser === p.author.username) {
      deleteBtn = '<button data-click="CommunityHub.deletePost" data-click-args=' + JSON.stringify([p.id]) + ' '
    ...
    /* Like */
    +         '<button data-click="CommunityHub.toggleLike" data-click-args=' + JSON.stringify([p.id]) + ' '
    ...
    /* Comment */
    +         '<button data-click="CommunityHub.openComments" data-click-args=' + JSON.stringify([p.id]) + ' '
    ...
    /* Share */
    +         '<button data-click="CommunityHub.sharePost" data-click-args=' + JSON.stringify([p.id, p.title || '']) + ' '
  ...
  function init() {
    ...
    if (feedRoot) {
      ...
      feedRoot.addEventListener("click", function(e) {
        ...
        if (btn) {
          var fnName = btn.dataset.click;
          var args = btn.dataset.clickArgs ? JSON.parse(btn.dataset.clickArgs) : [];
          var fn = window.CommunityHub[fnName];
          if (fn && fn.length > args.length) args.push(btn);
