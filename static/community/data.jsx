...
(function () {
  ...
  const FALLBACK_COLORS = {
    valorant: '#FF4655', 'counter-strike-2': '#F2A52B', cs2: '#F2A52B',
    'dota-2': '#C73B27', dota2: '#C73B27',
    'mobile-legends': '#FFD43B', mlbb: '#FFD43B',
    'pubg-mobile': '#F2A52B', pubgm: '#F2A52B',
    'free-fire': '#FF6B35', freefire: '#FF6B35',
    'ea-sports-fc-26': '#00FFAA', fc26: '#00FFAA',
    'efootball-2026': '#005CFF', efootball: '#005CFF',
    'call-of-duty-mobile': '#FFB400', codm: '#FFB400',
    'rocket-league': '#3DAEFF', rl: '#3DAEFF',
  };
  const shortFor = (slug, name) => {
    const map = {
      'valorant': 'VAL', 'counter-strike-2': 'CS2', 'dota-2': 'DOTA',
      'mobile-legends': 'MLBB', 'pubg-mobile': 'PUBG', 'free-fire': 'FF',
      'ea-sports-fc-26': 'FC26', 'efootball-2026': 'EF',
      'call-of-duty-mobile': 'CODM', 'rocket-league': 'RL',
    };
  ...
  function normPost(p, gameById) {
    ...
    return {
      ...
      image: firstMedia && firstMedia.type !== 'video'
        ? { url: firstMedia.url, alt: firstMedia.alt || '' }
        : undefined,
      clip: firstMedia && firstMedia.type === 'video'
        ? { url: firstMedia.url, duration: '', views: 0, caption: game ? game.name : '' }
        : undefined,
      team: p.team || null,
  ...
  const me = cfg.me || {
    id: 'u-anon', name: 'Guest', handle: 'guest',
    color1: '#7B2BFF', color2: '#00E5FF',
    team: null, verified: false, rank: null, country: '🇧🇩',
    avatar_url: null,
  };

  window.DC = {
    GAMES: [], GAME_BY_ID: {},
    USERS: [me], USER_BY_ID: { [me.id]: me },
    ME: me,
    MY_IDENTITIES: [{
      id: 'self', kind: 'user', user: me,
      canPost: !!cfg.isAuthenticated, role: 'You',
    }],
    TEAMS: [],
    MY_TOURNAMENTS: [],
    MY_PASSPORTS: [],
    ...
    NOTIFICATIONS: [],
    COMMENTS_BY_POST: {},
    SIDEBAR_STATS: { total_posts: 0, total_teams: 0, total_games: 0 },
    FEATURED_TEAMS: [],

    api: {
      fetchJSON, headersJSON, csrf,
      urls: URLS,
      ...
      loadUserTeams() {
        ...
        return fetchJSON(URLS.userTeams).then(d => {
          ...
          const ids = [{
            id: 'self', kind: 'user', user: window.DC.ME,
            canPost: !!cfg.isAuthenticated, role: 'You',
          }];
          teams.forEach(t => {
            ...
            ids.push({
              id: 'team-' + t._pk, kind: 'team', teamId: t.id,
              canPost: isCaptain || isCoLeader,
      ...
      loadFeed(opts) {
        if (!URLS.feed) return Promise.resolve({ posts: [] });
        ...
        return fetchJSON(url).then(d => {
          ...
          return { posts, page: d.page, has_next: d.has_next, total: d.total };
        });
      },
      ...
      toggleLike(postPk) {
        ...
        return fetchJSON(url, { method: 'POST', headers: headersJSON() });
      },

      listComments(postPk) {
        if (!URLS.comments) return Promise.resolve({ comments: [] });
      ...
      getPreferences() {
        if (!URLS.preferences) return Promise.resolve({ tweaks: null });
      ...
      savePreferences(edits) {
        ...
        return fetchJSON(URLS.preferences, {
          method: 'PATCH',
          headers: headersJSON(),
          body: JSON.stringify({ tweaks: edits }),
        });
      },
